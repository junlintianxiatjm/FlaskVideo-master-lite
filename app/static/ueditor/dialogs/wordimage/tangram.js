 ...
 * @meta standard
 * @see baidu.swf.createHTML,baidu.swf.getvideo
 */
...
/**
 * 获得flash对象的实例
 * @name baidu.swf.getvideo
 * @function
 * @grammar baidu.swf.getvideo(name)
 * @param {string} name flash对象的名称
 * @see baidu.swf.create
 * @meta standard
 * @returns {HTMLElement} flash对象的实例
 */
baidu.swf.getvideo = function (name) {
	var video = document[name], ret;
    return baidu.browser.ie == 9 ?
    	video && video.length ?
    		(ret = baidu.array.remove(baidu.lang.toArray(video),function(item){
...
baidu.flash._Base = (function(){
    ...
    function _render(options){
        ...
        return baidu.swf.getvideo(options.id);
    };
